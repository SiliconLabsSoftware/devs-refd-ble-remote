...
int main(void)
{
  ...
  sl_led_handler_deinit();

  communication_shutdown();

  if ((BOOTLOADER_OK == ret)
      || ((BOOTLOADER_ERROR_COMMUNICATION_DONE == ret)
          && (reset_getResetReason() != BOOTLOADER_RESET_REASON_BADAPP))) {
    reset_resetWithReason(BOOTLOADER_RESET_REASON_GO);
  } else {
    // An error occurred during the firmware upgrade process
    // and the current firmware is not valid.
    // Go into EM4 to wait for the user to manually enter the bootloader.
    EMU_EnterEM4();
  }
...
void SystemInit2(void)
{
  ...
  if (enterApp && (EMU->RSTCAUSE & EMU_RSTCAUSE_POR)) {
    sl_recovery_init();
    if (sl_recovery_verify_condition()) {
      // If the recovery condition is met, we want to enter the bootloader
      enterApp = false;
      verifyApp = false;
    }
    sl_recovery_deinit();
  ...
  // RMU_GetResetCause() description states:
  // "To be useful, the reset cause must be cleared by software before a new reset occurs"
  RMU_ResetCauseClear();
...
#pragma GCC diagnostic pop
#endif
#if defined(_SILICON_LABS_32B_SERIES_2) && defined(CRYPTOACC_PRESENT) && defined(FIH_ENABLE_DELAY)
  // fih_delay_init is applicable for vse devices
  fih_delay_init();
#endif
  // App should be verified
  if (verifyApp) {
#if defined(_SILICON_LABS_32B_SERIES_2) && defined(FIH_ENABLE_DELAY)
    fih_delay();
#endif
  ...
  if (enterApp) {
    ...
    // The necessary check of valid signature pointer for application at startOfAppSpace
