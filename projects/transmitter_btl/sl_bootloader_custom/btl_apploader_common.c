...
{
  uint32_t inactivity_timeout_start_tick;
  uint32_t led_blink_timeout_tick;
  ...
  inactivity_timeout_start_tick = sl_dwt_handler_get_cycle_count();
  led_blink_timeout_tick = sl_dwt_handler_get_cycle_count();
  ...
  while (1) {
    if (isConnected) {
      sl_apploader_run();
      if (started) {
        // Toggle LED with 10Hz
        if (sl_dwt_handler_is_elapsed_ms(led_blink_timeout_tick, SYS_CNF_LED_BLINK_PERIOD_OTA_MS)) {
          led_blink_timeout_tick = sl_dwt_handler_get_cycle_count();
          sl_led_toggle();
        }
      } else {
        // Toggle LED with 2 Hz
        if (sl_dwt_handler_is_elapsed_ms(led_blink_timeout_tick, SYS_CNF_LED_BLINK_PERIOD_CONNECTION_MS)) {
          led_blink_timeout_tick = sl_dwt_handler_get_cycle_count();
          sl_led_toggle();
        }
      }
    ...
    if (!isConnected && sl_dwt_handler_is_elapsed_ms(inactivity_timeout_start_tick, SYS_CNF_INCATIVITY_TIMEOUT_MS)) {
